...
const CouponCode = () => {
    ...
    const handle_Discount = () => {
        ...
        if (type === 'percentage') {
            ...
            if (!FlatDiscount(discount)) {
                alert('Flat discount only 0 to 500 rupees allowed.');
                return;
            }
    ...
    const handle_Submit = (e) => {
        ...
        if (duration === 'once' && generate) {
            alert('Coupon code only one time change duration is set to ONCE.');
            return;
        }
